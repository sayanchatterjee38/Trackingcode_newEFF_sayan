...
void plotHist2D() {
  ...
  TFile *f = new TFile("Sep17_2021_negtrk_generaltrack_algocuts_phicent.root");

  char ndir[256] = "HITrackCorrections";
  
  // sim-to-reco hists
  TH2F *hSim = (TH2F*) f->Get(Form("%s/hsim2D",ndir));  
  TH2F *hEff = (TH2F*) f->Get(Form("%s/heff2D",ndir)); 
  TH2F *hMul = (TH2F*) f->Get(Form("%s/hmul2D",ndir)); 

  /*
  hSim->GetYaxis()->SetRangeUser(60 , 80);
  hEff->GetYaxis()->SetRangeUser(60 , 80);
  hMul->GetYaxis()->SetRangeUser(60 , 80);
  */

  // reco-to-sim hists
  TH2F *hRec = (TH2F*) f->Get(Form("%s/hrec2D",ndir)); 
  TH2F *hFak = (TH2F*) f->Get(Form("%s/hfak2D",ndir)); 
  TH2F *hSec = (TH2F*) f->Get(Form("%s/hsec2D",ndir)); 

  /*
  hRec->GetYaxis()->SetRangeUser(60 , 80);
  hFak->GetYaxis()->SetRangeUser(60 , 80);
  hSec->GetYaxis()->SetRangeUser(60 , 80);
  */
  ...
  TFile *f1 = new TFile("NegativeTrk_2018PbPb_phicent_GenTrkalgo.root","RECREATE");
  rEff->Write();
  rFak->Write();
  f1->Close();
